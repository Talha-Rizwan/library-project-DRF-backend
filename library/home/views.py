...
from home.constants import CLOSED_STATUS, ADMIN_EMAIL, DELAYED_REQUEST_DAYS, LIBRARIAN_EMAIL
...
class UserRequestTicket(generics.ListCreateAPIView):
    queryset = RequestTicket.objects.filter(status="P")
    ...
    def perform_update(self, serializer):
        ...
        if 'status' in serializer.validated_data:
            if serializer.validated_data['status'] == APPROVED_STATUS:
                print(f'the name is :  {self.request.data}')
                new_book = Book(
