...
class LibrarianDetailRequestTestCase(APITestCase):
    ...
    def test_get_request_with_wrong_id(self):
        ...
        response = self.client.get(f'{self.url}{1000}/', format=FORMAT)
    ...
    def test_librarian_approve_request(self):
        ...
        response = self.client.put(f'{self.url}{self.Requests[0].id}/',data={"status": "A"}, format=FORMAT)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_librarian_reject_request(self):
        ...
        response = self.client.put(f'{self.url}{self.Requests[0].id}/',data={"status": "R"}, format=FORMAT)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_request_with_wrong_input(self):
        ...
        response = self.client.put(f'{self.url}{self.Requests[0].id}/',data={"status": "Incorrect"}, format=FORMAT)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_librarain_non_existant_request(self):
        '''Test to update a request that doesnot exist.'''
        response = self.client.put(f'{self.url}100/',data={"status": "R"}, format=FORMAT)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
